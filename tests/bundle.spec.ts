...
describe('dsh-codeaudit bundle', () => {
  ...
  it('ships the codeaudit agent preset with the core plugin row and submit-only subagents', () => {
    ...
    // The official Yakit reference skills ship in the box: no runtime network needed.
    expect(existsSync(`${presetDir}skills/webfuzzer-hotpatch.md`)).toBe(true)
    expect(existsSync(`${presetDir}skills/yaklang-syntax.md`)).toBe(true)
