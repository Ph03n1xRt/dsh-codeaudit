...
3. 每条能从代码推导出完整请求的漏洞都要构造 POC，遵循 Yakit Web Fuzzer 的 HTTP POC 语法：
   纯报文、可用 fuzztag；加密/签名/时间戳等动态值用 {{yak(...)}} 占位并把热加载函数写入 pocScript；
   无环境发送时静态构造并在 pocNote 注明「静态构造，未经发送验证」，有环境则发送后注明「已动态验证」；
