...
- **可重放 POC**：finding 可附 `poc` 字段——复现该漏洞的完整 HTTP raw（上限 16KB，动态验证后由子代理写入），
  UI 一键复制/下载，报告中原样输出；poc 为空即表示仅静态结论。
...
| 子视图 | 内容 |
|---|---|
| **探索链路** | 整张审计图（React Flow），**树状泳道布局**：任务卡片在顶部居中，**每个意图独占一条竖直泳道**，它的证据、漏洞依次挂在它自己正下方——漏洞永远是"鉴权绕过意图"那棵子树底部的"鉴权绕过漏洞"，不存在全局类型列堆积；推导产生的新意图占据相邻泳道。为大规模链路另有三重压缩：**意图聚合折叠**——默认只显示 任务→意图→漏洞 骨架，每个意图卡片带「证据 N · 漏洞 M」计数徽章，点徽章按需展开该意图的证据（折叠时证据的 supports/derived_from 边自动重挂到所属意图，高层语义不丢；小图 ≤15 节点自动全展开）；**「仅漏洞链路」过滤**隐去无结论的死胡同探索；缩放 0.05–4 倍 + 右下角可拖拽 **MiniMap**；**纵向(自上而下)/横向(自左向右)** 生长方向切换。点节点看详情，含代码位置与冻结代码片段 |
| **漏洞发现** | 按严重级排序的卡片列表；每条两个下钻：**完整漏洞链路**抽屉——任务 → 意图 → 逐条证据（kind 徽章 + 位置 + 代码片段）→ 漏洞结论；**yakit POC** 抽屉——冻结在漏洞记录里的完整 HTTP raw（请求行+头+体），可一键复制或下载 `.txt`，直接粘贴 Yakit/Burp 重放（仅静态结论时显示空态提示） |
...
每次审计只需粘贴下面这段，改两处：目标路径、范围行。

```
审计【/目标/代码库/路径】（技术栈自行识别并写入 engagement.stack）。

审计目标：全量安全审计。按「模块 × 漏洞类」矩阵拆分意图并发委派子代理，覆盖：
SQL/命令/表达式注入、反序列化、鉴权与越权（含水平/垂直）、文件上传下载与路径穿越、
SSRF、XSS、CSRF、密钥硬编码与弱加密、危险配置、依赖组件已知漏洞（对照依赖清单逐一核查）。

执行要求：
1. 先测绘再深挖：入口点、路由表、依赖、配置全部登记为资产，再拆意图；
2. 每条漏洞必须给出 file:line 位置、关键代码片段、至少一条证据；已排查 sanitizer/框架
   防护的标 confirmed，存疑的标 suspected 并写明未验证点；
3. 每条能从代码推导出完整请求的漏洞都要构造可重放的 HTTP POC（纯 HTTP raw）：
   无环境发送时可静态构造并在 pocNote 注明「静态构造，未经发送验证」；
   有可用环境（测试服/本地起服）则实际发送验证后注明「已动态验证」；
4. POC 的参数生成规则、占位符含义、前置条件写 pocNote，绝不混入报文；
5. 漏洞按严重级排序，逐条给修复建议；全部意图收敛或收益递减时出最终报告；
6. 构造 POC 前先用 skill 工具加载可用技能作参考：codeaudit-methodology（构造规范）、
   yak（Yakit 官方语法，若已拉取）。

范围：【只审 src/main，排除 test/、前端/、node_modules/】
```

可选加强（往执行要求里追加）：

- 聚焦某类：`优先深挖鉴权与注入，其余类型快速过`；
- 有测试环境：`目标已在 http://x.x.x.x:8080 部署可用，动态验证在该地址进行，验证流量控制在只读操作`。

注意：静态构造的 POC 未经发送验证，重放失败时以说明卡（pocNote）里的规则为准手动修正。
...
- **yak**（自动补装）— Yakit 官方技能（[yaklang/yak-skills](https://github.com/yaklang/yak-skills)），Yakit POC 语法与生成参考。插件首次挂载时检测预设技能目录，没有就从官方源拉取 SKILL.md 放入预设层；离线/失败静默跳过，下次挂载自动重试。想刷新版本：删除 `<插件目录>/preset/codeaudit/skills/yak.md` 再重启即可；不想自动拉取，把环境变量 `CODEAUDIT_SKIP_YAK_SKILL` 置为 `1`。
