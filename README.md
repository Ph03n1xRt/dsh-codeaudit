...
| 子视图 | 内容 |
|---|---|
| **探索链路** | 整张审计图（React Flow），**固定语义层级**：任务 → 意图 → 证据 → 漏洞 逐层加深（漏洞永远在其意图下方两层、证据层之下，无论推导链多长），同层节点按所属意图相邻聚簇；为大规模链路另有三层压缩：**意图聚合折叠**——默认只显示 任务→意图→漏洞 骨架（层级自动压缩不留空列），每个意图卡片带「证据 N · 漏洞 M」计数徽章，点徽章按需展开该意图的证据（折叠时证据的 supports/derived_from 边自动重挂到所属意图，高层语义不丢；小图 ≤15 节点自动全展开）；**「仅漏洞链路」过滤**隐去无结论的死胡同探索；缩放范围 0.05–4 倍 + 右下角可拖拽 **MiniMap**；**横向/纵向**布局切换。点节点看详情，含代码位置与冻结代码片段 |
